namespace KProxy
{
    ...
    internal class HTTPSServerHello
    {
        ...
        internal bool LoadFromStream(Stream oNS)
        {
            ...
            switch (num)
            {
                ...
                case 0x15:
                {
                    ...
                    //KProxyApplication.Log.LogFormat("Got an alert from the server!\n{0}", new object[] { Utilities.ByteArrayToHexView(buffer2, 8) });
                    return false;
                }
                default:
        ...
        public override string ToString()
        {
            ...
            {
                builder.Append("The data sent represents an SSLv2-compatible ServerHello handshake. In v2, the ~client~ selects the active cipher after the ServerHello, when sending the Client-Master-Key message. Kavprot Proxy only parses the handshake.\n\n");
            }
            else
            {
