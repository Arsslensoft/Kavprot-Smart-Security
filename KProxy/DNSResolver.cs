namespace KProxy
{
    ...
    internal class DNSResolver
    {
        ...
        private static int MSEC_DNS_CACHE_LIFETIME = KProxyApplication.Prefs.GetInt32Pref("KProxy.network.timeouts.dnscache", 0x249f0);

        static DNSResolver()
        {
            KProxyApplication.Janitor.assignWork(new SimpleEventHandler(DNSResolver.ScavengeCache), 0x7530);
        }
        ...
        private static IPAddress[] trimAddressList(IPAddress[] arrResult)
        {
            ...
            {
                if (!list.Contains(arrResult[i]) && (KPCONFIG.bEnableIPv6 || (arrResult[i].AddressFamily == AddressFamily.InterNetwork)))
                {
