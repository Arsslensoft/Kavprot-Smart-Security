namespace KProxy
{
    ...
    internal class Winsock
    {
        ...
        private static int FindPIDForConnection(int iTargetPort, uint iAddressType)
        {
            ...
            if (0x7a == GetExtendedTcpTable(zero, ref dwTcpTableLength, false, iAddressType, TcpTableType.OwnerPidConnections, 0))
            {
                try
                {
                    ...
                    //KProxyApplication.Log.LogFormat("GetExtendedTcpTable() returned error #{0}", new object[] { Marshal.GetLastWin32Error().ToString() });
                    return 0;
                }
                finally
                {
            ...
            //KProxyApplication.Log.LogFormat("Initial call to GetExtendedTcpTable() returned error #{0}", new object[] { Marshal.GetLastWin32Error().ToString() });
        Label_0125:
            return 0;
        }
        ...
        private static int FindPIDForPort(int iTargetPort)
        {
            ...
            {
                //KProxyApplication.Log.LogFormat("KProxy.Network.TCPTable> Unable to call IPHelperAPI function: {0}", new object[] { exception.Message });
            }
            return 0;
        }
