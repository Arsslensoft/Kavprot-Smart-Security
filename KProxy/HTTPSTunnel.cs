                ...
                if (count > 0)
                {
                    if ((this._mySession.responseBodyBytes == null) || (this._mySession.responseBodyBytes.LongLength == 0L))
                    {
                        try
                        {
                            ...
                            {
                                this._mySession.responseBodyBytes = Encoding.UTF8.GetBytes("This is a CONNECT tunnel, through which encrypted HTTPS traffic flows. To view the encrypted sessions inside this tunnel, ensure that the Tools > KProxy Options > HTTPS > Decrypt HTTPS traffic option is checked.\n\n" + hello.ToString() + "\n");
