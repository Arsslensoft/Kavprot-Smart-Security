namespace KProxy
{
    ...
    internal class PipePool
    {
        ...
        internal PipePool()
        {
            MSEC_PIPE_POOLED_LIFETIME = KProxyApplication.Prefs.GetInt32Pref("KProxy.network.timeouts.serverpipe.reuse", 0x1d4c0);
            this.thePool = new Dictionary<string, Queue<ServerPipe>>();
            KProxyApplication.Janitor.assignWork(new SimpleEventHandler(this.ScavengeCache), 0xea60);
        }
        ...
        {
            if ((KPCONFIG.bReuseServerSockets && ((oPipe.sPoolKey != null) && (oPipe.sPoolKey.Length >= 2))) && ((oPipe.ReusePolicy != PipeReusePolicy.MarriedToClientPipe) && (oPipe.ReusePolicy != PipeReusePolicy.NoReuse)))
            {
                Queue<ServerPipe> queue;
                lock (this.thePool)
                {
