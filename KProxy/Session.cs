namespace KProxy
{
    ...
    public class Session : IDisposable
    {
       public void Dispose()
        ...
        [CodeDescription("Client port attached to KProxy.")]
        ...
        [CodeDescription("KProxy-internal flags set on the session.")]
        ...
        internal Session(ClientPipe clientPipe, ServerPipe serverPipe)
        {
            ...
            this.bBufferResponse = KProxyApplication.Prefs.GetBoolPref("KProxy.ui.rules.bufferresponses", false);
            ...
            if (KPCONFIG.bDebugSpew)
            {
                if (handler == null)
                {
                    handler = delegate (object s, StateChangeEventArgs ea) {
                        KProxyApplication.DebugSpew(string.Format("onstatechange>#{0} moving from state '{1}' to '{2}' {3}", new object[] { this.id.ToString(), ea.oldState, ea.newState, Environment.StackTrace }));
                    };
                }
        ...
        public Session(HTTPRequestHeaders oRequestHeaders, byte[] arrRequestBody)
        {
            ...
            this.Timers.ClientConnected = this.Timers.ClientBeginRequest = this.Timers.KProxyGotRequestHeaders = DateTime.Now;
        ...
        public Session(byte[] arrRequest, byte[] arrResponse, SessionFlags oSF)
        {
            ...
            {
                arrResponse = Encoding.ASCII.GetBytes("HTTP/1.1 0 KProxy GENERATED - RESPONSE DATA WAS MISSING\r\n\r\n");
            }
            ...
            string sData = KPCONFIG.oHeaderEncoding.GetString(arrRequest, 0, num) + "\r\n\r\n";
            string sHeaders = KPCONFIG.oHeaderEncoding.GetString(arrResponse, 0, num3) + "\r\n\r\n";
        ...
        private bool _executeObtainRequest()
        {
            ...
            else
            {
                ...
                if (KPCONFIG.bDebugSpew)
                {
                    KProxyApplication.DebugSpew(string.Format("Session ID #{0} for request read from {1}.", this.m_requestID, this.oRequest.pipeClient));
                }
            ...
            {
                KProxyApplication.HandleHTTPError(this, SessionFlags.ProtocolViolationInRequest, true, false, "This HTTP method requires a request body.");
            }
            if (this.oFlags.ContainsKey("X-Original-Host"))
            {
                ...
                if (string.Empty == str)
                {
                    KProxyApplication.HandleHTTPError(this, SessionFlags.ProtocolViolationInRequest, false, false, "HTTP/1.1 Request was missing the required HOST header.");
                }
                else
                {
                    KProxyApplication.HandleHTTPError(this, SessionFlags.ProtocolViolationInRequest, false, false, string.Format("Request's Host header does not match Host specified in URI.\n\nURL Host:\t{0}\nHeader Host:\t{1}", str, this.oRequest.headers["Host"]));
                }
            }
            this.state = SessionStates.AutoTamperRequestBefore;
            KProxyApplication.DoBeforeRequest(this);
        ...
        {
            string str;
            if ((!KPCONFIG.bMITM_HTTPS || this.oFlags.ContainsKey("x-no-decrypt")) || ((KPCONFIG.oHLSkipDecryption != null) && KPCONFIG.oHLSkipDecryption.ContainsHost(this.PathAndQuery)))
            {
                HTTPSTunnel.CreateTunnel(this);
                this.state = SessionStates.Done;
                KProxyApplication.DoAfterSessionComplete(this);
                return true;
            }
            ...
            try
            {
                ...
                else if (!this.bypassGateway)
                {
                    ...
                    point = KProxyApplication.oProxy.FindGatewayForOrigin("https", this.PathAndQuery);
                ...
                else
                {
                    ...
                    {
                        this.oRequest.FailSession(0x1f6, "Kavprot Proxy - DNS Lookup Failed", "Kavprot Proxy: DNS Lookup for " + Utilities.HtmlEncode(str) + " failed.");
                        return true;
                    }
                    ...
                    else
                    {
                        ...
                        KProxyApplication.DoBeforeReturningError(this);
                ...
                this.oResponse.headers.Add("Timestamp", DateTime.Now.ToString("HH:mm:ss.fff"));
                this.oResponse.headers.Add("KProxyGateway", "Direct");
                ...
                KProxyApplication.DoBeforeResponse(this);
                this.state = SessionStates.Done;
                KProxyApplication.DoAfterSessionComplete(this);
            ...
            {
                KProxyApplication.ReportException(exception2, "HTTPS Interception failure");
                this.oRequest.pipeClient = null;
            }
            return true;
        }
        ...
        private bool _isDirectRequestToKProxy()
        {
            if (this.port != KPCONFIG.ListenPort)
            {
                return false;
            }
            string str = this.hostname.ToLower();
            if ((!(str == "127.0.0.1") && !(str == "localhost")) && (!(str == "localhost.") && !(str == KPCONFIG.sMachineNameLowerCase)))
            {
                return (str == "[::1]");
            }
            return true;
        ...
        {
            if (((!KPCONFIG.bReuseClientSockets || !this._bAllowClientPipeReuse) || (this.oResponse.headers.ExistsAndEquals("Connection", "close") || this.oRequest.headers.ExistsAndEquals("Connection", "close"))) || this.oRequest.headers.ExistsAndEquals("Proxy-Connection", "close"))
            {
                return false;
            }
        ...
        {
            KProxyApplication.CallLocalHost(this);
            this.state = SessionStates.Aborted;
        }

        private void _returnLogFileResponse()
        {
            this.SetBitFlag(SessionFlags.ResponseGeneratedByKProxy, true);
            ...
            this.state = SessionStates.Aborted;
            KProxyApplication.DoResponseHeadersAvailable(this);
        ...
        private void _returnRootCert()
        {
            ...
            byte[] buffer = KProxy.Properties.Resources.Root;
            if (buffer != null)
            {
            ...
            else
            {
                ...
                this.utilSetResponseBody("No root certificate was found. Have you enabled HTTPS traffic decryption in KProxy yet?".PadRight(0x200, ' '));
            }
            this.state = SessionStates.Done;
            KProxyApplication.DoResponseHeadersAvailable(this);
            this.ReturnResponse(false);
        ...
        private void _ReturnSelfGeneratedCONNECTTunnel(string sHostname)
        {
            ...
            this.oResponse.headers.Add("Timestamp", DateTime.Now.ToString("HH:mm:ss.fff"));
            this.oResponse.headers.Add("KProxyGateway", "AutoResponder");
            this.responseBodyBytes = Encoding.UTF8.GetBytes("This is a KProxy-generated response to the client's request for a CONNECT tunnel.\n\n");
        ...
        public void Abort()
        {
            try
            {
                if (this.m_state < SessionStates.Done)
                {
                    ...
                    this.CloseSessionPipes(true);
                    this.oFlags["x-KProxy-Aborted"] = "true";
                    this.state = SessionStates.Aborted;
                    this.ThreadResume();
                }
            }
        ...
        private void CloseSessionPipes(bool bNullThemToo)
        {
            if (KPCONFIG.bDebugSpew)
            {
                KProxyApplication.DebugSpew(string.Format("CloseSessionPipes() for Session #{0}", this.id));
            }
        ...
        internal void Execute(object objThreadState)
        {
            ...
            {
                KProxyApplication.ReportException(exception, "Uncaught Exception in Session #" + this.id.ToString());
            }
        }
        ...
        private void InnerExecute()
        {
            ...
            {
                if (this.isAnyFlagSet(SessionFlags.ResponseGeneratedByKProxy))
                {
                    KProxyApplication.DoResponseHeadersAvailable(this);
                }
                goto Label_0338;
            ...
            if (this.oFlags.ContainsKey("x-replywithfile"))
            {
                ...
                if (this.LoadResponseFromFile(this.oFlags["x-replywithfile"]) && this.isAnyFlagSet(SessionFlags.ResponseGeneratedByKProxy))
                {
                    KProxyApplication.DoResponseHeadersAvailable(this);
                }
            ...
            {
                KProxyApplication.HandleHTTPError(this, SessionFlags.ProtocolViolationInRequest, true, false, "HTTP Request specified an invalid port number.");
            }
            if (this._isDirectRequestToKProxy())
            {
                if (KPCONFIG.bHookWithPAC && this.oRequest.headers.RequestPath.EndsWith("/Log.txt"))
                {
                ...
                this.host = KPCONFIG.sReverseProxyHostname + ":" + KPCONFIG.iReverseProxyForPort.ToString();
            }
        ...
        Label_01A5:
            ...
            if (!this.oResponse.ReadResponse())
            {
                ...
                {
                    KProxyApplication.DebugSpew("Failed to read server response. Aborting.");
                    if (this.state != SessionStates.Aborted)
                    {
                        this.oRequest.FailSession(0x1f8, "KProxy - Receive Failure", "ReadResponse() failed: The server did not return a response for this request.");
                    }
                    this.CloseSessionPipes(true);
                    this.state = SessionStates.Aborted;
                    return;
                }
                KProxyApplication.DebugSpew("[" + this.id.ToString() + "] ServerSocket Reuse failed. Restarting fresh.");
            ...
            else
            {
                ...
                {
                    KProxyApplication.HandleHTTPError(this, SessionFlags.ProtocolViolationInResponse, true, true, string.Format("Content-Length mismatch: Response Header indicated {0:N0} bytes, but server sent {1:N0} bytes.", num, this.responseBodyBytes.LongLength));
                }
            }
        Label_0338:
            ...
            if ((((this.oResponse.headers.HTTPResponseCode > 0x12b) && (this.oResponse.headers.HTTPResponseCode < 0x134)) && (this.oFlags.ContainsKey("x-Builder-MaxRedir") && this.oResponse.headers.Exists("Location"))) && this.isValidAutoRedir(this.fullUrl, this.oResponse["Location"]))
            {
                ...
                this.nextSession.SetBitFlag(SessionFlags.RequestGeneratedByKProxy, true);
            ...
            else
            {
                if ((this._isResponseMultiStageAuthChallenge() && this.oFlags.ContainsKey("x-AutoAuth")) && !this.oFlags.ContainsKey("x-AutoAuth-Failed"))
                {
                    ...
                    try
                    {
                        ...
                        {
                            //KProxyApplication.Log.LogFormat("Cannot get TransportContext. You may need to upgrade to a later .NET Framework. {0}", new object[] { exception.Message });
                        }
                    ...
                    {
                        //KProxyApplication.Log.LogFormat("Automatic authentication was unsuccessful. {0}", new object[] { exception2.Message });
                    }
                }
                bool flag3 = false;
        ...
        public bool LoadMetadata(Stream strmMetadata)
        {
            ...
            try
            {
                ...
                Label_00E1:
                    ...
                    if (attribute != null)
                    {
                        this.Timers.KProxyGotRequestHeaders = XmlConvert.ToDateTime(attribute, XmlDateTimeSerializationMode.RoundtripKind);
                    }
                    ...
                    attribute = reader.GetAttribute("KProxyBeginRequest");
                    if (attribute != null)
                    {
                        this.Timers.KProxyBeginRequest = XmlConvert.ToDateTime(attribute, XmlDateTimeSerializationMode.RoundtripKind);
                    }
                    ...
                    if (attribute != null)
                    {
                        this.Timers.KProxyGotResponseHeaders = XmlConvert.ToDateTime(attribute, XmlDateTimeSerializationMode.RoundtripKind);
                    }
                ...
                if (this.Timers.KProxyBeginRequest.Ticks < 1L)
                {
                    this.Timers.KProxyBeginRequest = this.Timers.ServerGotRequest;
                }
                if (this.Timers.KProxyGotRequestHeaders.Ticks < 1L)
                {
                    this.Timers.KProxyGotRequestHeaders = this.Timers.ClientBeginRequest;
                }
                if (this.Timers.KProxyGotResponseHeaders.Ticks < 1L)
                {
                    this.Timers.KProxyGotResponseHeaders = this.Timers.ServerBeginResponse;
                }
        ...
        public bool LoadRequestBodyFromFile(string sFilename)
        {
            ...
            sFilename = Utilities.EnsurePathIsAbsolute(KPCONFIG.GetPath("Requests"), sFilename);
        ...
        public bool LoadResponseFromFile(string sFilename)
        {
            ...
            this.BitFlags |= SessionFlags.ResponseGeneratedByKProxy;
        ...
        internal bool ReturnResponse(bool bForceClientServerPipeAffinity)
        {
            ...
            try
            {
                if ((this.oRequest.pipeClient != null) && this.oRequest.pipeClient.Connected)
                {
                    ...
                    else
                    {
                        if (KPCONFIG.bDebugSpew)
                        {
                            KProxyApplication.DebugSpew(string.Format("KProxy.network.clientpipereuse> Closing client socket since bReuseClientSocket was false after returning [{0}]", this.url));
                        }
                        this.oRequest.pipeClient.End();
                        flag = true;
                    }
            ...
            catch (Exception exception)
            {
                if (KPCONFIG.bDebugSpew)
                {
                    KProxyApplication.DebugSpew(string.Format("Write to client failed for Session #{0}; exception was {1}", this.id, exception.ToString()));
                }
                this.state = SessionStates.Aborted;
            }
            this.oRequest.pipeClient = null;
        ...
        public bool SaveRequestBody(string sFilename)
        {
            ...
            {
                KProxyApplication.DoNotifyUser(exception.Message + "\n\n" + sFilename, "Save Failed");
                return false;
            }
        }
        ...
        [CodeDescription("Save HTTP response body to KProxy Captures folder.")]
        public bool SaveResponseBody()
        {
            string path = KPCONFIG.GetPath("Captures");
        ...
        internal bool ShouldBeHidden()
        {
            ...
            if (KProxyApplication.Prefs.GetBoolPref("KProxy.filters.ephemeral.debugmode", false))
            {
            ...
            if (KPCONFIG.bReportHTTPErrors && this.isAnyFlagSet(SessionFlags.ProtocolViolationInResponse | SessionFlags.ProtocolViolationInRequest))
            {
                return false;
            }
            if (this.isFlagSet(SessionFlags.RequestGeneratedByKProxy) && !this.oFlags["ui-hide"].Contains("stealth"))
            {
                return false;
            }
            return true;
        ...
        public void utilCreateResponseAndBypassServer()
        {
            ...
            this.oResponse = new ServerChatter(this, "HTTP/1.1 200 OK\r\nContent-Length: 0\r\n\r\n");
            this.responseBodyBytes = new byte[0];
            this.oFlags["x-KProxy-Generated"] = "utilCreateResponseAndBypassServer";
            this.BitFlags |= SessionFlags.ResponseGeneratedByKProxy;
            this.bBufferResponse = true;
            this.state = SessionStates.AutoTamperResponseBefore;
        }
        ...
        public bool utilDecodeRequest()
        {
            ...
            {
                KProxyApplication.ReportException(exception, "utilDecodeRequest failed for Session #" + this.id.ToString());
                return false;
            }
            return true;
        }
        ...
        public bool utilDecodeResponse()
        {
            ...
            {
                KProxyApplication.ReportException(exception, "utilDecodeResponse failed for Session #" + this.id.ToString());
                return false;
            }
            return true;
        }
        ...
        public void WriteMetadataToStream(Stream strmMetadata)
        {
            ...
            writer.WriteAttributeString("GotRequestHeaders", XmlConvert.ToString(this.Timers.KProxyGotRequestHeaders, XmlDateTimeSerializationMode.RoundtripKind));
            ...
            writer.WriteAttributeString("KProxyBeginRequest", XmlConvert.ToString(this.Timers.KProxyBeginRequest, XmlDateTimeSerializationMode.RoundtripKind));
            ...
            writer.WriteAttributeString("GotResponseHeaders", XmlConvert.ToString(this.Timers.KProxyGotResponseHeaders, XmlDateTimeSerializationMode.RoundtripKind));
        ...
        public SessionFlags BitFlags
        {
            ...
            internal set
            {
                if (KPCONFIG.bDebugSpew && (value != this._bitFlags))
                {
                    KProxyApplication.DebugSpew(string.Format("Session #{0} bitflags adjusted from {1} to {2} @ {3}", new object[] { this.id, this._bitFlags, value, Environment.StackTrace }));
                }
        ...
        [CodeDescription("Returns the Address used by the client to communicate to KProxy.")]
        ...
        [CodeDescription("Returns the port used by the client to communicate to KProxy.")]
        ...
        public string SuggestedFilename
        {
            get
            {
                ...
                if (((sFilename.Length > 0) && (sFilename.Length < 0x40)) && (sFilename.Contains(".") && (sFilename.LastIndexOf('.') == sFilename.IndexOf('.'))))
                {
                    ...
                    string str6 = KProxyApplication.Prefs.GetBoolPref("KProxy.session.prependIDtosuggestedfilename", false) ? "{0}_{1}{2}" : "{1}{2}";
