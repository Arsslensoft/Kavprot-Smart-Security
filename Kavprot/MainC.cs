...
namespace Kavprot
{
    public partial class MainC : UserControl
    {
        ...
        void pfrm_PSLChanged(object sender, EventArgs e)
        {
            try
            {
               ...
               Forms.frm.sflb.Text = "SF : " + scannedfiles;
                if(SettingsManager.OptimizeGUI)
                    Forms.frm.metroStatusBar1.Refresh();
            }
            catch (Exception ex)
            {
        ...
        internal void LoadStats()
        {
            ...
            else if (!(FileSystemMonitor.Runing && WebMonitor.IsRuning && NetworkMonitor.Runing && Firewall.Runing))
            {
