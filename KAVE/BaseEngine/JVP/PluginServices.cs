				...
				if (pluginType.IsPublic) //Only look at public types
				{
					if (!pluginType.IsAbstract)  //Only look at non-abstract types
					{
						...
						if (typeInterface != null)
						{
							...
							newPlugin.Instance = (Core)Activator.CreateInstance(pluginAssembly.GetType(pluginType.ToString()));
		...
		public class AvailablePlugins : System.Collections.CollectionBase
		{
			...
			public Types.AvailablePlugin Find(string pluginNameOrPath)
			{
				...
				foreach (Types.AvailablePlugin pluginOn in this.List)
				{
					...
					if ((pluginOn.Instance.ProductID.Equals(pluginNameOrPath)) || pluginOn.AssemblyPath.Equals(pluginNameOrPath))
					{
						toReturn = pluginOn;
						break;		
					}
