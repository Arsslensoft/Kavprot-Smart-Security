...
#endif
using KCompress.Sdk;
using KCompress.Sdk.Compression.Lzma;
#if MONO
using KCompress.Mono.COM;
#endif
...
namespace KCompress
{
    ...
    /// </summary>
    /// <example>
    /// var compr = new KCompressCompressor();
    /// compr.CompressDirectory(@"C:\Dir", @"C:\Archive.7z");
    /// </example>
    public sealed partial class KCompressCompressor
#if UNMANAGED
        : KCompressBase
#endif
    {
        ...
        private OutArchiveFormat _archiveFormat = OutArchiveFormat.KCompress;
        ...
        /// <summary>
        /// Initializes a new instance of the KCompressCompressor class. 
        /// </summary>
        public KCompressCompressor()
        {            
            try
            {
#if !WINCE
            ...
            {
                /*throw new KCompressCompressionFailedException(
                    "Attempted to get TEMP environment variable but registry access was not allowed (security settings on your machine). You must call KCompressCompressor constructor overload with your own temporary path.");
                 */
                throw new KCompressCompressionFailedException(
                    "Path.GetTempPath() threw a System.Security.SecurityException. You must call KCompressCompressor constructor overload with your own temporary path.");
            }
            CommonInit();   
        }
        ...
        public KCompressCompressor(string temporaryPath)
        {
            ...
            if (!Directory.Exists(TempFolderPath))
            {
                ...
                {
                    throw new KCompressCompressionFailedException("The specified temporary path is invalid.");
                }
            }
            CommonInit();
        }
        ...
        {
            IInArchive inArchive = KCompressLibraryManager.InArchive(
            ...
            using (ArchiveOpenCallback openCallback = GetArchiveOpenCallback())
            {
                ...
                {
                    if (
                        !ThrowException(null, new KCompressArchiveException("Can not update the archive: Open() failed.")))
                    {
                        return null;
                    }
                }
        ...
        private bool MethodIsValid(CompressionMethod method)
        {
            ...
            switch (_archiveFormat)
            {
                ...
                case OutArchiveFormat.KCompress:
            ...
            switch (_archiveFormat)
            {
                ...
                default:
                    ISetProperties setter = CompressionMode == CompressionMode.Create && _updateData.FileNamesToModify == null
                                            ?   (ISetProperties) KCompressLibraryManager.OutArchive(
                                                    _archiveFormat, this)
                                            :   (ISetProperties) KCompressLibraryManager.InArchive(
                                                    Formats.InForOutFormats[_archiveFormat], this);
                    if (setter == null)
                    {
                    ...
                    if (_volumeSize > 0 && ArchiveFormat != OutArchiveFormat.KCompress)
                    {
                    ...
                    if (EncryptHeaders && _archiveFormat == OutArchiveFormat.KCompress &&
        ...
        private static void CheckCommonRoot(string[] files, ref int commonRootLength)
        {
            ...
            {
                throw new KCompressInvalidFileNamesException("invalid common root.");
            }
        ...
        /// <summary>
        /// Occurs when all files information was determined and KCompressCompressor is about to start to compress them.
        /// </summary>
            ...
            Stream archiveStream, int commonRootLength, string password, params string[] fileFullNames)
        {
            ...
            try
            {
                ...
                using ((sequentialArchiveStream = GetOutStream(archiveStream)) as IDisposable)
                {
                    ...
                    using ((inArchiveStream = GetInStream()) as IDisposable)
                    {
                        ...
                        {
                            KCompressLibraryManager.LoadLibrary(this, _archiveFormat);
                            outArchive = KCompressLibraryManager.OutArchive(_archiveFormat, this);
                        }
                        else
                        {
                            // Create IInArchive, read it and convert to IOutArchive
                            KCompressLibraryManager.LoadLibrary(
                            ...
                            try
                            {
                                if (files != null) //ReSharper
                                    CheckedExecute(
                                        ...
                                        KCompressCompressionFailedException.DEFAULT_MESSAGE, auc);
                            }
                            finally
                            {
            ...
            finally
            {
                if (CompressionMode == CompressionMode.Create || !_compressingFilesOnDisk)
                {
                    KCompressLibraryManager.FreeLibrary(this, _archiveFormat);
                }
                else
                {
                    KCompressLibraryManager.FreeLibrary(this, Formats.InForOutFormats[_archiveFormat]);
...
#endif
        {
            ...
            {
                throw new KCompressInvalidFileNamesException("the specified directory is empty!");
            }
            if (recursion)
            {
            ...
            try
            {
                ...
                using ((sequentialArchiveStream = GetOutStream(outStream)) as IDisposable)
                {
                    using (ArchiveUpdateCallback auc = GetArchiveUpdateCallback(inStream, password))
                    {
                        ...
                        {
                            CheckedExecute(
                                KCompressLibraryManager.OutArchive(_archiveFormat, this).UpdateItems(
        ...
        {
            ClearExceptions();
            if (!KCompressLibraryManager.ModifyCapable)
            {
                throw new KCompressLibraryException("The specified 7zip native library does not support this method.");
            }
            ...
            try
            {
                using (var extr = new KCompressExtractor(archiveName))
                {
                    _updateData = new UpdateData();
