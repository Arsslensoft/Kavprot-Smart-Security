...
namespace KCompress
{
    ...
    /// </summary>
    /// <example>
    /// using (var extr = new KCompressExtractor(@"C:\Test.7z"))
    /// {
    ///     extr.ExtractArchive(@"C:\TestDirectory");
    /// }
    /// </example>
    public sealed partial class KCompressExtractor
#if UNMANAGED
        : KCompressBase, IDisposable
#endif
    {
#if UNMANAGED
        ...
        private void Init(string archiveFullName)
        {
            ...
            KCompressLibraryManager.LoadLibrary(this, _format);
            try
            {
                _archive = KCompressLibraryManager.InArchive(_format, this);
            }
            catch (KCompressLibraryException)
            {
                KCompressLibraryManager.FreeLibrary(this, _format);
                throw;
            }
        ...
        /// <summary>
        /// Initializes a new instance of KCompressExtractor class.
        /// </summary>
        /// <param name="archiveStream">The stream to read the archive from.
        /// Use KCompressExtractor(string) to extract from disk, though it is not necessary.</param>
        /// <remarks>The archive format is guessed by the signature.</remarks>
        public KCompressExtractor(Stream archiveStream)
        {
            Init(archiveStream);
        }
        ...
        /// Instead, use KCompressExtractor(Stream archiveStream), that constructor
        /// automatically detects the archive format.</param>
        public KCompressExtractor(Stream archiveStream, InArchiveFormat format)
        {
            _format = format;
            Init(archiveStream);
        }
        ...
        public KCompressExtractor(string archiveFullName)
        {
        ...
        /// <param name="archiveFullName">The archive full file name.</param>
        /// <param name="format">Manual archive format setup. You SHOULD NOT normally specify it this way.
        /// Instead, use KCompressExtractor(string archiveFullName), that constructor
        /// automatically detects the archive format.</param>
        public KCompressExtractor(string archiveFullName, InArchiveFormat format)
        {
            _format = format;
            Init(archiveFullName);
        }
        ...
        public KCompressExtractor(string archiveFullName, string password)
            : base(password)
        {
            Init(archiveFullName);
        }
        ...
        /// <param name="password">Password for an encrypted archive.</param>
        /// <param name="format">Manual archive format setup. You SHOULD NOT normally specify it this way.
        /// Instead, use KCompressExtractor(string archiveFullName, string password), that constructor
        /// automatically detects the archive format.</param>
        public KCompressExtractor(string archiveFullName, string password, InArchiveFormat format)
            : base(password)
        {
            _format = format;
            Init(archiveFullName);
        ...
        /// <param name="password">Password for an encrypted archive.</param>
        /// <remarks>The archive format is guessed by the signature.</remarks>
        public KCompressExtractor(Stream archiveStream, string password)
            : base(password)
        {
            Init(archiveStream);
        }
        ...
        /// <param name="password">Password for an encrypted archive.</param>
        /// <param name="format">Manual archive format setup. You SHOULD NOT normally specify it this way.
        /// Instead, use KCompressExtractor(Stream archiveStream, string password), that constructor
        /// automatically detects the archive format.</param>
        public KCompressExtractor(Stream archiveStream, string password, InArchiveFormat format)
            : base(password)
        {
            _format = format;
            Init(archiveStream);
        ...
        public bool IsSolid
        {
            get
            {
                ...
                throw new KCompressException("_isSolid == null");
            }
        }
        ...
        public uint FilesCount
        {
            get
            {
                ...
                throw new KCompressException("_filesCount == null");
            }
        }
        ...
        private void DisposedCheck()
        {
            if (_disposed)
            {
                throw new ObjectDisposedException("KCompressExtractor");
            }
#if !WINCE
            ...
            ArchiveOpenCallback openCallback)
        {
            if (!_opened)
            {
                ...
                {
                    if (!ThrowException(null, new KCompressArchiveException()))
                    {
                        return false;
                    }
                }
        ...
        /// </summary>
        /// <exception cref="KCompress.KCompressArchiveException"/>
                    ...
                    if (_filesCount != 0)
                    {
                        ...
                        try
                        {
                            ...
                            for (uint i = 0; i < _filesCount; i++)
                            {
                                ...
                                {
                                    ThrowException(null, new KCompressArchiveException("probably archive is corrupted."));
                                }
                            }
        ...
        /// <summary>
        /// Releases the unmanaged resources used by KCompressExtractor.
        /// </summary>
        public void Dispose()
        {
            if (_asynchronousDisposeLock)
            {
                throw new InvalidOperationException("KCompressExtractor instance must not be disposed " +
                ...
                using (var aec = GetArchiveExtractCallback("", (int)_filesCount, null))
                {
                    try
                    {
                        CheckedExecute(
                            ...
                            KCompressExtractionFailedException.DEFAULT_MESSAGE, aec);
                    }
                    finally
                    {
