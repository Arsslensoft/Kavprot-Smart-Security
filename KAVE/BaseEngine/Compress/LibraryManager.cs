...
namespace KCompress
{
    ...
    /// </summary>
    internal static class KCompressLibraryManager
    {        
#if !WINCE && !MONO
        /// <summary>
        ...
        /// </remarks>
        private static string _libraryFileName = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "7z.dll");
#endif
#if WINCE 		
            ...
            ref LibraryFeature? features, LibraryFeature testedFeature)
        {
            try
            {
                var compr = new KCompressCompressor {ArchiveFormat = format, CompressionMethod = method};
                ...
                using (var inStream = new MemoryStream())
                {
                    ...
                    using (var outStream = new MemoryStream())
                    {
                        CompressionBenchmark(inStream, outStream, 
                            OutArchiveFormat.KCompress, CompressionMethod.Lzma,
                            ref _features, LibraryFeature.Compress7z);
                        CompressionBenchmark(inStream, outStream,
                            OutArchiveFormat.KCompress, CompressionMethod.Lzma2,
                            ref _features, LibraryFeature.Compress7zLZMA2);
                        int i = 0;
                        if (CompressionBenchmark(inStream, outStream,
                            OutArchiveFormat.KCompress, CompressionMethod.BZip2,
                            ref _features, _features.Value))
                        {
                            i++;
                        }
                        if (CompressionBenchmark(inStream, outStream,
                            OutArchiveFormat.KCompress, CompressionMethod.Ppmd,
                            ref _features, _features.Value))
                        {
                            i++;
            ...
            lock (_libraryFileName)
            {
                ...
                if (_inArchives[user][format] == null)
                {
                    ...
                    if (_modulePtr == IntPtr.Zero)
                    {
                        ...
                        if (_modulePtr == IntPtr.Zero)
                        {
                            throw new KCompressLibraryException();
                        }
                    }
...
#else
						result = KCompress.Mono.Factory.CreateInterface<IInArchive>(user, classID, interfaceId);
#endif
                    }
                    catch
                    {
                        throw new KCompressLibraryException("Your 7-zip library does not support this archive type.");
                    }
 ...
 typeof(IOutArchive).GUID;
                    ...
                    try
                    {
                    	...
                    	NativeMethods.CreateCOMObject(ref classID, ref interfaceId, out result);
#else
						result = KCompress.Mono.Factory.CreateInterface<IOutArchive>(classID, interfaceId, user);
#endif
                    }
                    catch
                    {
